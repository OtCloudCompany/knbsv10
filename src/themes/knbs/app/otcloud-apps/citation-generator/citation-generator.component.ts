...
export class CitationGeneratorComponent implements OnInit {
  ...
  private static customTemplatesRegistered = false;
  ...
  ngOnInit(): void {
    ...
    try {
      ...
      console.error('Failed to generate citations for item', this.object?.uuid, error);
    }
  }

  extractDateParts(dateStr: string): number[] {
    return dateStr.split('-').map(part => parseInt(part, 10)).filter(part => !isNaN(part));
  }

  /**
   * Converts a DSpace author string into a CSL-JSON name. DSpace stores personal
   * names as "Family, Given" but corporate/institutional authors (e.g. "Kenya
   * National Bureau of Statistics (KNBS)") have no such comma. Without this,
   * citation-js's name parser mis-splits corporate names as if they were
   * personal ones (e.g. producing "of Statistics (KNBS), K. N. B.").
   */
  private toCslName(name: string): { literal: string } | { family: string; given: string } {
  ...
  /**
   * Maps a DSpace dc.type value to a CSL item type. Defaults to 'report', which
   * fits KNBS's mostly statistical-report/working-paper output better than
   * citation-js's own 'article-journal' fallback.
   */
  private mapDspaceTypeToCsl(dspaceType: string): string {
    const normalized = (dspaceType || '').toLowerCase();
    const mapping: { match: RegExp; type: string }[] = [
      { match: /article/, type: 'article-journal' },
      { match: /book chapter|chapter/, type: 'chapter' },
      { match: /book/, type: 'book' },
      { match: /thesis|dissertation/, type: 'thesis' },
      { match: /conference|presentation/, type: 'paper-conference' },
      { match: /dataset|data set/, type: 'dataset' },
      { match: /working paper|technical report|report/, type: 'report' },
    ];
    return mapping.find(({ match }) => match.test(normalized))?.type ?? 'report';
  }

  generateCitations() {
    const dateIssued = this.object.firstMetadataValue(['dc.date.issued']);
    const publisher = this.object.firstMetadataValue(['dc.publisher']);
    const cite = new Cite({
      type: this.mapDspaceTypeToCsl(this.object.firstMetadataValue(['dc.type'])),
      title: this.object.firstMetadataValue(['dc.title']),
      author: this.object.allMetadataValues(['dc.contributor.author']).map(name => this.toCslName(name)),
      ...(dateIssued && { issued: { 'date-parts': [this.extractDateParts(dateIssued)] } }),
      publisher,
    ...
    // Register the custom CSL styles using the content as text (once per app lifetime)
  ...
  /**
   * The custom CSL styles are global to citation-js's plugin registry, so they
   * only need to be added once per application lifetime, not on every call to
   * generateCitations() (e.g. once per item page render).
   */
  private registerCustomTemplates() {
    if (CitationGeneratorComponent.customTemplatesRegistered) {
      return;
    }
    const templates = plugins.config.get('@csl').templates;
    templates.add('mla', this.getMLATemplate());
    templates.add('iso690-2', this.getISO690Template());
    templates.add('chicago', this.getChicagoTemplate());
    CitationGeneratorComponent.customTemplatesRegistered = true;
  }

  convertToCSV(cite) {
    const data = cite.data;
    const headers = ['Type', 'Title', 'Author', 'Date', 'Journal', 'Publisher', 'URL'];
    ...
    const rows = data.map(entry => {
      const authors = (entry.author || [])
        .map(a => a.literal ?? `${a.given} ${a.family}`)
        .join('; ');
